...
const makeGate = (profile, canEdit) => ({fields, title, filter=false, Cell = (f) => <div style={{marginTop: "1em"}}>{profile[f]}</div>, Cells = {}, style={}}) => {
  ...
  const EditButton = canEdit  // EditButton is either empty string or an actual edit button depending on canEdit
    ? ({fields}) => <Editor profile={profile} fields={fields} title={title}/>
    : (props) => "";
